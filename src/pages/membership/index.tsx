...
const MembershipPage = () => {
  ...
  useFetchMembershipsQuery(
    {
      ...
      limit: 4,
      // ground_type: selectedVenue.length > 0 ? JSON.stringify(selectedGroundType) : null,
    },
    {
  ...
  useEffect(() => {
    if (detailsPageId) {
      const foundMembership = memberships.find((i) => i.id === detailsPageId)
        ?.ground.name;
      if (foundMembership) {
        dispatch(setLocationArr(foundMembership));
      }
    } else {
      dispatch(resetLocationArr());
    }
  }, [detailsPageId, memberships]);
            ...
            <div className="flex flex-col px-5 lg:px-20 xl:px-40 mt-8 lg:mt-0 h-full">
              ...
              {!!detailsPageId ? (
                isJoinAcademy ? (
                  ...
                  <>
                    <MembershipDetails membershipId={detailsPageId} />
                  </>
                )
              ) : (
