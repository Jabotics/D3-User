...
import { useMyMembershipsQuery } from "@/store/actions/slices/membershipSlice";
...
const Memberships = () => {
  ...
  const { joinedMemberships } = useAppSelector(
    (state: RootState) => state.membership
  );
  return (
    <TooltipProvider>
      {joinedMemberships && joinedMemberships.length >= 0 ? (
        <div className="hidden flex-col gap-4 w-full sm:flex">
          <span className="inline-block text-sm font-base">My Memberships</span>
          <div id="bookings" className="w-full flex flex-col gap-4">
            ...
            <div className="w-full h-[45vh] overflow-x-hidden overflow-y-auto">
              {joinedMemberships.map((item, index) => {
                return (
                  <div
                    key={index}
